...
def create_env_widgets() -> tuple:
    """Creates input widgets for BASE_URL and API_TOKEN and returns them."""
    base_url_widget = widgets.Text(description='BASE_URL:', placeholder='Enter the Base URL')

    api_key_widget = widgets.Password(description='API_TOKEN:', placeholder='Enter your API Token')
...
def save_env_file(base_url: str, api_key: str) -> None:
    ...
    with Path('.env').open('w', encoding='utf-8') as file:
        ...
        if api_key.strip():  # Only write API_TOKEN if it's not empty
            ...
            print('✅ Successfully created the .env file without API_TOKEN defined!')


def display_env_input() -> None:
    """Displays widgets for BASE_URL and API_TOKEN input and provides a save button."""
